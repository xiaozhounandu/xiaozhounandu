...
-- 插入测试用户 (密码: admin123, user123 - BCrypt加密)
...
CREATE TABLE `customer` (
  ...
  `status` TINYINT NOT NULL DEFAULT 1 COMMENT '状态: 0-删除, 1-正常, 2-已成交, 3-流失',
...
-- 管理员账号:
--   用户名: admin
--   密码: admin123  (BCrypt加密: $2a$10$N.ZmdGzV7IOTH8w2j7C8rKeNW8pC8t9q.WO2vPn6lzK5cB4eS7R0Cq)
--
-- 销售经理:
--   用户名: manager
--   密码: admin123  (同上)
...
-- BCrypt特点:
-- - 相同密码每次加密结果不同（自动加盐）
-- - 可通过 BCrypt.matches(rawPassword, encryptedPassword) 验证
-- - 推荐在Spring Boot中使用 BCryptPasswordEncoder
-- ==================================================
